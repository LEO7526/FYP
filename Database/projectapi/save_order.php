...
function resolveOrderTimezone(array $input): DateTimeZone {
    ...
    try {
        ...
        error_log('save_order: invalid timezone "' . $candidate . '", fallback to ' . DEFAULT_ORDER_TIMEZONE);
...
if (!$window['allowed']) {
    error_log('save_order: order window blocked; tz=' . $window['timezone'] . ', server_time=' . $window['server_time']);
