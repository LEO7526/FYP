...
try {
    ...
    if (!$couponId) {
        die("No coupon selected.");
    }

    // 撈出 coupon 所需積分
    $stmt = $pdo->prepare("SELECT points_required
                           FROM coupons 
                           WHERE coupon_id = ?");
    ...
    if (!$member) {
        die("Member points record not found.");
    }

    $currentPoints = (int)$member['coupon_point'];
    $requiredPoints = (int)$coupon['points_required'];
    ...
    if ($currentPoints >= $requiredPoints) {
        ...
        echo "<p>Points deducted: <strong>{$requiredPoints}</strong></p>";
        ...
        echo "<p>You need <strong>{$requiredPoints}</strong> points to redeem this coupon.</p>";
