...
try {
    ...
    if ($couponId) {
        $stmt = $pdo->prepare("SELECT type, discount_amount, item_category, cr.min_spend 
                           FROM coupons c 
                           JOIN coupon_rules cr ON c.coupon_id = cr.coupon_id 
                           WHERE c.coupon_id = ?");
        $stmt->execute([$couponId]);
        $coupon = $stmt->fetch(PDO::FETCH_ASSOC);
    }

    if ($coupon && $coupon['min_spend'] > $totalPrice) {
        header("Location: select_coupon.php?error=not_enough_spend");
        exit;
    }
                    ...
                    if ($package_id) {
                        ...
                        if ($package) {
                            ...
                            foreach ($package_items as $pid) {
                                $stmt = $pdo->prepare("SELECT mit.item_name, ptt.type_name 
                                               FROM menu_item mi 
                                               JOIN menu_item_translation mit ON mi.item_id = mit.item_id 
                                               JOIN package_dish pd ON pd.item_id = mi.item_id
                                               JOIN package_type_translation ptt ON pd.type_id = ptt.type_id AND ptt.type_language_code = 'en'
                                               WHERE mi.item_id = ? AND mit.language_code = 'en' AND pd.package_id = ?");
                                $stmt->execute([$pid, $package_id]);
                                $item = $stmt->fetch();
                                if ($item) {
