...
if (isset($_GET['error']) && $_GET['error'] === 'not_enough_spend') {
    echo "<p style='color:red;'>不滿足最低消費，請重新選擇優惠券。</p>";
}
...
try {
    ...
    $stmt = $pdo->prepare(" SELECT cr.coupon_id, ct.title, ct.description FROM coupon_redemptions cr JOIN coupon_translation ct ON cr.coupon_id = ct.coupon_id WHERE cr.cid = ?  AND cr.is_used = 0 AND ct.language_code = 'en'");
    $stmt->execute([$cid]);
...
<div class="container">
    ...
    <form method="post" action="customer_finalize_order.php">
        <?php foreach ($item_ids as $index => $id): ?>
            ...
            <input type="hidden" name="spice_levels[]" value="<?= htmlspecialchars($spice_levels[$index] ?? '') ?>">
            <input type="hidden" name="remarks[]" value="<?= htmlspecialchars($remarks[$index] ?? '') ?>">
        <?php endforeach; ?>
        <?php if ($package_id): ?>
            <input type="hidden" name="package_id" value="<?= htmlspecialchars($package_id) ?>">
        <?php endif; ?>
        <?php foreach ($package_items as $pid): ?>
            <input type="hidden" name="package_selected_items[]" value="<?= htmlspecialchars($pid) ?>">
        <?php endforeach; ?>
        <?php foreach ($package_item_modifiers as $modifier): ?>
            <input type="hidden" name="package_item_modifiers[]" value="<?= htmlspecialchars($modifier) ?>">
        <?php endforeach; ?>
        ...
        <?php if ($couponRedemptions): ?>
            ...
            <?php endforeach; ?>
            <label style="display:block; margin:8px 0;">
                <input type="radio" name="coupon" value="" required>
                <strong>Do not use any coupon</strong>
            </label>
