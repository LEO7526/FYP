...
if ($bookings) {
    echo "<h4>Your booking record：</h4><ul>";
    foreach ($bookings as $b) {
        echo "<li>
            <strong>booking ID：</strong> {$b['bid']}<br>
            Date：{$b['bdate']}，Time：{$b['btime']}，People：{$b['pnum']}，Seats：{$b['tid']}<br>
            <form method='post' class='edit-form' style='display:inline;'>
                <input type='hidden' name='action' value='edit'>
                <input type='hidden' name='bid' value='{$b['bid']}'>
                <button type='submit'>Edit</button>
            </form>
            <form method='post' class='cancel-form' style='display:inline;' onsubmit='return confirm(\"Comfrim to cancel？\");'>
