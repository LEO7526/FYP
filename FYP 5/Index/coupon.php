    ...
    if (!isset($_COOKIE['user'])) {
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT cr.coupon_id, ct.title, ct.description 
                       ...
                       WHERE cr.cid = ? AND cr.is_used = 0 AND ct.language_code = 'en'");
            ...
            if ($coupons){
                echo '<div class="coupon-container-wrapper">';
                foreach ($coupons as $c){
                    $couponId = htmlspecialchars($c['coupon_id']);
                    $title = htmlspecialchars($c['title']);
                    $desc = htmlspecialchars($c['description']);
                    $points = (int)$c['points_required'];

                    $canRedeem = ($memberPoints >= $points);

                    // 如果不足，增加 disabled class
                    $cardClass = $canRedeem ? "coupon-card" : "coupon-card disabled";

                    echo '<div class="'.$cardClass.'">';
                    echo '<h3>'.$title.'</h3>';
                    echo '<p>'.$desc.'</p>';
                    echo '<p class="points">Points Required: '.$points.'</p>';

                    if ($canRedeem) {
                        echo '<button type="button" class="redeem-btn" onclick="openRedeemPopup(\''.$couponId.'\', \''.$title.'\')">Redeem</button>';
                    } else {
                        echo '<button type="button" class="redeem-btn" disabled>Not enough points</button>';
                    }
